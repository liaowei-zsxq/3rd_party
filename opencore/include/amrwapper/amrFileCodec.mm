...
static void SkipToPCMAudioData(FILE *fpwave) {
    LYH_RIFFHEADER riff;
    LYH_FMTBLOCK fmt;
    LYH_XCHUNKHEADER chunk;
    LYH_WAVEFORMATX wfx;
    int bDataBlock = 0;

    // 1. Read the RIFF header
    fread(&riff, 1, sizeof(LYH_RIFFHEADER), fpwave);

    // 2.  Read the FMT chunk - if fmt.nFmtSize>16, read the remaining MATX
    fread(&chunk, 1, sizeof(LYH_XCHUNKHEADER), fpwave);
    if (chunk.nChunkSize > 16) {
        fread(&wfx, 1, sizeof(LYH_WAVEFORMATX), fpwave);
    } else {
        memcpy(fmt.chFmtID, chunk.chChunkID, 4);
        fmt.nFmtSize = chunk.nChunkSize;
        fread(&fmt.wf, 1, sizeof(LYH_WAVEFORMAT), fpwave);
    }
...
// bps = 8 --> 8 bits
//       16 --> 16 bits
int LYH_EncodeWAVEFileToAMRFile(const char *pchWAVEFilename, const char *pchAMRFileName, int nChannels, int nBitsPerSample) {
    FILE *fpwave;
    FILE *fpamr;
...
static void WriteWAVEFileHeader(FILE *fpwave, int nFrame) {
    ...
    riff.nRiffSize = 4                                // WAVE
                     + sizeof(LYH_XCHUNKHEADER)       // fmt
                     + sizeof(LYH_WAVEFORMATX)        // LYH_WAVEFORMATX
                     + sizeof(LYH_XCHUNKHEADER)       // DATA
                     + nFrame * 160 * sizeof(short);  //
    strcpy(tag, "WAVE");
    memcpy(riff.chRiffFormat, tag, 4);
    fwrite(&riff, 1, sizeof(LYH_RIFFHEADER), fpwave);
    ...
    chunk.nChunkSize = sizeof(LYH_WAVEFORMATX);
    fwrite(&chunk, 1, sizeof(LYH_XCHUNKHEADER), fpwave);
    memset(&wfx, 0, sizeof(LYH_WAVEFORMATX));
    wfx.nFormatTag = 1;
    ...
    fwrite(&wfx, 1, sizeof(LYH_WAVEFORMATX), fpwave);
...
int LYH_DecodeAMRFileToWAVEFile(const char *pchAMRFileName, const char *pchWAVEFilename) {
    FILE *fpamr = NULL;
    FILE *fpwave = NULL;
    char magic[8];
    void *destate;
    ...
    //    ZLog(@"documentPath=%@", documentPath);
    //
